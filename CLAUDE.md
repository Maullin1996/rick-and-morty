...
Clean Architecture, organized **by feature**, not by layer. Each feature under `lib/feature/<name>/` has its own `data/`, `domain/`, `presentation/`. `auth` follows the full stack like `home`/`character` (datasource -> repository -> use case), just with no domain **entity** — a `bool` session flag doesn't need one, see **Auth & Firebase**. `favorite` also follows the full stack (Firestore-backed, see **Persistence**) but has no `*Model`/adapter of its own — it reuses `character`'s `Character` entity directly, since a Firestore favorite document has the exact same shape as `Character.toJson()`.
...
Existing features: `home` (list/search/filter), `character` (detail), `favorite` (per-user favorites persisted in Cloud Firestore, gated behind login), `auth` (Login/Register screens backed by real `FirebaseAuth`), `user` (bottom-nav "Usuario" tab, placeholder profile page).
...
### Persistence
Favorites are per-user, stored in **Cloud Firestore** (`cloud_firestore` package) under `users/{uid}/favorites/{characterId}`, one document per character (doc id = `character.id.toString()`, doc body = `Character.toJson()` plus a server-generated `addedAt` timestamp used to order the list newest-first). `feature/favorite` follows the same layered stack as `auth`/`character`: `FavoriteRemoteDatasourceImpl` (`data/datasources/`) is the only place touching `cloud_firestore` directly, `FavoriteRepositoryImpl` (`data/repositories/`) resolves the current uid via `AuthUseCase.currentUserId` (exposed all the way down from `AuthRemoteDatasourceImpl._firebaseAuth.currentUser?.uid`, mirroring `isLoggedIn`) and maps exceptions to `Failure`s the same way `CharacterRepositoryImpl` does, and `FavoriteUseCase` is a thin passthrough. Security is enforced by `firestore.rules` at the project root (`users/{userId}/**` readable/writable only when `request.auth.uid == userId`) — **this file must be pasted manually into the Firebase Console** (Firestore Database → Rules); there is no `firebase` CLI wired into this repo/environment to deploy it automatically.

`FavoriteNotifier` (`feature/favorite/presentation/providers/favorite_provider.dart`) keeps its public API unchanged from the old local-storage version (`List<Character>` state, synchronous `toggleCharacter`/`addCharacter`/`removeCharacter`/`isFavorite`/`clearAll`) so `characters_grid.dart`, `character_page.dart`, and `favorite_page.dart` never call it differently. Internally, `build()` does `ref.watch(isLoggedInProvider)`: logging in triggers a one-shot fetch from Firestore (`FavoriteUseCase.getFavorites()`) that populates the state once it resolves, logging out resets it to `[]` — this is a Riverpod-reactive replacement for a manual `authStateChanges` subscription. `addCharacter`/`removeCharacter` update `state` immediately (optimistic) and fire-and-forget the Firestore write (not awaited, matching the previous `SharedPreferences` behavior — write failures are silent). There is no realtime sync (no Firestore snapshot listener); favorites only reload on the next login. Every favorite-toggle call site (`characters_grid.dart`, `character_page.dart`, `favorite_page.dart`) is still gated by `requireAuth(context, ref)` (`feature/auth/presentation/helpers/require_auth.dart`) — without a session it shows an `AppSnackBar` with an "Iniciar sesión" action instead of toggling.
...
- `feature/auth/domain/repositories/auth_repository.dart` + `domain/usecase/auth_use_case.dart` are thin, same convention as `CharacterRepository`/`CharacterUseCase` — no domain **entity** though, since there's no meaningful domain data beyond a `bool` session flag and a `Stream<bool>`. They also expose `currentUserId` (`String?`, from `_firebaseAuth.currentUser?.uid`), added so `feature/favorite` can scope Firestore reads/writes to the logged-in user without importing `firebase_auth` itself — see **Persistence**.
...
- `favorite`'s unit tests (`data/repositories/favorite_repository_impl_test.dart`, `domain/usecase/favorite_use_case_test.dart`, `presentation/provider/favorite_provider_test.dart`) mock `FavoriteRemoteDatasource`/`FavoriteRepository`/`AuthUseCase`/`FavoriteUseCase` with mocktail — no `fake_cloud_firestore` package, no real Firestore touched. Any test that stubs a method taking a `Character` with mocktail's `any()` needs `registerFallbackValue(FakeCharacter())` (a `class FakeCharacter extends Fake implements Character {}`) in `setUpAll`, or mocktail throws `Bad state: ... registerFallbackValue was not previously called`. Widget tests (`favorite_page_test.dart`, `home_page_test.dart`, `character_page_test.dart`) don't need any of this — they override `favoriteProvider` wholesale with a local `FavoriteNotifier` subclass (e.g. `SpyFavoriteNotifier`/`FakeFavoriteNotifier`), bypassing Firestore entirely.
